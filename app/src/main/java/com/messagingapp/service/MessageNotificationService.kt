...
class MessageNotificationService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        // Always call startForeground immediately in onCreate to avoid ANR on Android 8+
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        // If the service gets killed, restart it but don't re-deliver last intent
        return START_STICKY
    }
    ...
    override fun onDestroy() {
        // Mark offline when service is stopped (logout / app close)
        // Use a separate standalone scope so the call completes even as onDestroy finishes
