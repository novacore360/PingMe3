...
fun AppNavigation() {
    ...
    AnimatedContent(
        ...
        when (currentScreen) {
            ...
            AppScreen.Auth -> {
                // key(sessionKey) ensures AuthViewModel is fresh after logout
                key(sessionKey) {
            ...
            AppScreen.Home -> {
                // key(sessionKey) ensures all ViewModels are fresh on each login
                key(sessionKey) {
                    HomeScreen(onLogout = {
                        // Increment key forces all ViewModels to be recreated
                        sessionKey++
                        screen = AppScreen.Auth
                    })
                }
