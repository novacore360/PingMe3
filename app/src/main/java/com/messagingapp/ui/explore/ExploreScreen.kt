    ...
    Column(Modifier.fillMaxSize().background(Color.Transparent)) {
        ...
        OutlinedTextField(
            ...
            colors = OutlinedTextFieldDefaults.colors(
                focusedTextColor = GlassColors.textPrimary, unfocusedTextColor = GlassColors.textPrimary,
                focusedContainerColor = Color(0x1AFFFFFF), unfocusedContainerColor = Color(0x0DFFFFFF),
                focusedBorderColor = GlassColors.primary.copy(0.5f), unfocusedBorderColor = GlassColors.glassBorder
            )
        )
...
fun UserCard(user: UserProfile, onMessage: () -> Unit, onPreview: () -> Unit) {
    Column(
        Modifier.glassCard(18.dp).clickable(onClick = onPreview).padding(16.dp),
        horizontalAlignment = Alignment.CenterHorizontally,
        verticalArrangement = Arrangement.spacedBy(8.dp)
    ) {
        Box {
            AvatarCircle(name = user.nickname, avatarUrl = user.avatarUrl, size = 56)
            if (user.isOnline) {
                Box(
                    Modifier.size(14.dp).align(Alignment.BottomEnd)
                        .background(Color(0xFF0A0E1A), CircleShape).padding(2.dp)
                ) { Box(Modifier.fillMaxSize().background(GlassColors.success, CircleShape)) }
            }
        }
        Text(user.nickname, style = MaterialTheme.typography.titleMedium, color = GlassColors.textPrimary,
            maxLines = 1, overflow = TextOverflow.Ellipsis)
        Text("@${user.username}", style = MaterialTheme.typography.labelSmall, color = GlassColors.textTertiary,
            maxLines = 1, overflow = TextOverflow.Ellipsis)
        if (user.isOnline) {
    ...
    Box(
        ...
        Column(
            Modifier.fillMaxWidth()
                ...
                .border(1.dp, Brush.linearGradient(listOf(Color(0x40FFFFFF), Color(0x10FFFFFF))),
