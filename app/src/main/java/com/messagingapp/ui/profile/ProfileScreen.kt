...
class ProfileViewModel : ViewModel() {
    ...
    // Used to force image reload after upload (cache-bust key)
    private val _imageReloadKey = MutableStateFlow(0L)
    val imageReloadKey = _imageReloadKey.asStateFlow()
        ...
        viewModelScope.launch {
            ...
            repo.uploadAvatar(uid, bytes, "image/jpeg")
                .onSuccess { url ->
                    ...
                    _imageReloadKey.value = System.currentTimeMillis()
                    loadProfile()
                }
...
fun ProfileScreen(
    ...
    // When logged out state changes to true, navigate away
    ...
    Scaffold(
        ...
        Column(
            ...
            Box(
                ...
                // Cover image - use imageReloadKey to force cache bypass after upload
                if (!profile?.coverUrl.isNullOrBlank()) {
                    val coverUrl = profile!!.coverUrl!!
                    // key(imageReloadKey) forces recomposition and a fresh ImageRequest
                    key(imageReloadKey) {
                        val coverRequest = ImageRequest.Builder(context)
                            .data("$coverUrl?v=$imageReloadKey")
            ...
            Box(
                ...
                Box(modifier = Modifier.align(Alignment.CenterStart)) {
                    ...
                    Box(
                        ...
                        // Use imageReloadKey to force avatar reload too
                        key(imageReloadKey) {
                            AvatarCircle(
                                name = profile?.nickname ?: "?",
                                avatarUrl = profile?.avatarUrl?.let { "$it?v=$imageReloadKey" },
                                size = 82
                            )
                        }
                    }
