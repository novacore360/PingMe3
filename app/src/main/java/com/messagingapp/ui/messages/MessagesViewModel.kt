...
class MessagesViewModel : ViewModel() {
    ...
    fun openConversation(conversationId: String, otherUserId: String) {
        // Cancel old listeners and fully reset state to avoid data bleed between accounts
        realtimeJob?.cancel()
        presenceJob?.cancel()
        ...
        viewModelScope.launch {
            ...
            messageRepo.getMessages(conversationId)
                ...
                .onFailure { e ->
                    _chatState.update { it.copy(otherUser = user, isLoading = false, error = e.message) }
                }
        }
        ...
        realtimeJob = viewModelScope.launch {
            messageRepo.listenToMessages(conversationId).collect { msg ->
                _chatState.update { s ->
                    ...
                    // Remove matching pending message when real message arrives
                    val updatedPending = s.pendingMessages.filter { pending ->
                        msg.senderId != currentUserId || !pending.content.equals(msg.content, ignoreCase = false)
                    }
    ...
    fun sendMessage(content: String, conversationId: String) {
        ...
        val pending = PendingMessage(localId = localId, content = content, createdAt = now, status = "sending")
        _chatState.update { it.copy(pendingMessages = it.pendingMessages + pending, replyTo = null) }
            ...
            if (result.isFailure) {
                ...
                _chatState.update { s ->
                    s.copy(pendingMessages = s.pendingMessages.map {
                        if (it.localId == localId) it.copy(status = "failed") else it
                    })
                }
            } else {
                // On success, the realtime listener will remove the pending message
                // But if realtime is slow, mark as sent after a short delay
                delay(3000)
                _chatState.update { s ->
                    s.copy(pendingMessages = s.pendingMessages.filter { it.localId != localId })
                }
            }
    ...
    fun retryPending(localId: String, conversationId: String) {
        val pending = _chatState.value.pendingMessages.find { it.localId == localId } ?: return
        // Remove the failed one and re-send
        _chatState.update { s -> s.copy(pendingMessages = s.pendingMessages.filter { it.localId != localId }) }
        sendMessage(pending.content, conversationId)
    }
