...
fun HomeScreen(onLogout: () -> Unit) {
    ...
    val listState by messagesVm.listState.collectAsState()
    val totalUnread = listState.totalUnreadCount
    ...
    val overlayPermLauncher = rememberLauncherForActivityResult(
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M || Settings.canDrawOverlays(context)) {
            val bubbleSvc = Intent(context, ChatBubbleService::class.java)
            ContextCompat.startForegroundService(context, bubbleSvc)
        }
    }
    ...
    Box(
        ...
        Row(
            modifier = Modifier
                ...
                .border(
                    1.dp,
                    Brush.linearGradient(listOf(Color(0x50FFFFFF), Color(0x0AFFFFFF))),
            ...
            items.forEach { item ->
                ...
                val badge = if (item.route == Screen.Messages.route && unreadCount > 0) unreadCount else 0
                NavBarItem(
                    item = item,
                    selected = selected,
                    badgeCount = badge,
...
@Composable
fun NavBarItem(item: NavItem, selected: Boolean, badgeCount: Int = 0, onClick: () -> Unit) {
    Box(
        modifier = Modifier
